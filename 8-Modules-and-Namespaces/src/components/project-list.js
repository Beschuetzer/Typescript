...
class ProjectList extends base_component_js_1.Component {
    constructor(type) {
        super('project-list', 'app', project_model_js_1.InsertLocation.end, `${type}-projects`);
        this.type = type;
    ...
    dropHandler(event) {
        project_state_js_1.projectState.moveProject(event.dataTransfer.getData('text/plain'), this.type === 'active' ? project_model_js_1.ProjectStatus.Active : project_model_js_1.ProjectStatus.Finished);
    }
    ...
    configure() {
        project_state_js_1.projectState.addListener((projects) => {
            const relevantProjects = projects.filter(project => {
                if (this.type === 'active')
                    return project.status === project_model_js_1.ProjectStatus.Active;
                return project.status === project_model_js_1.ProjectStatus.Finished;
            });
